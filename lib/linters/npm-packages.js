...
module.exports = function (packageJsonPath, packageJsonRaw, config, callback) {
    ...
    exec('npm list --depth=1 --json', {
        ...
        for (var packageName in packageJson.dependencies) {
            expectedPackages[packageName] = packageJson.dependencies[packageName];
        }
        for (packageName in packageJson.devDependencies) {
            expectedPackages[packageName] = packageJson.devDependencies[packageName];
        }
